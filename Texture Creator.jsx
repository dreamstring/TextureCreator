(function () {
	...
	function isIndex(value, length) {
		...
		return (
			!!length &&
			or(type === 'number', reIsUint.test(value)) &&
			value > -1 &&
			value % 1 == 0 &&
			value < length
		);
	...
	var isContainer = createIsElementFlag(reContainer),
		isListItemContainer = createIsElementFlag(reListItemContainer),
		isNativeContainer = createIsElementFlag(reNativeContainer),
		isNativeControl = createIsElementFlag(reNativeControl),
		isSelectableElement = createIsElementFlag(reSelectableElement);
	function addContainer(container, value, flag, collector) {
		return flag == 'node'
			? addNodeContainer(container, value, flag, collector)
			: addGeneralContainer(container, value, flag, collector);
	}
	function addControl(container, value, key, collector) {
		var element = isListItemContainer(container.type)
			? addListItem(container, value)
			: addGeneralControl(container, value, key);
	...
	function addGeneralContainer(container, value, flag, collector) {
		...
		var newContainer = nativeAddContainer(
			container,
			flag,
			assignElementParam(value, flag)
		);
	...
	function baseEachElement(containers, accumulator, breaker, predicate) {
		return some(containers, function (container) {
			...
			return isDone
				? true
				: baseEachElement(result, accumulator, breaker, predicate);
		});
	}
	...
	function getElementsByName() {
		...
		baseEachElement([this], result, breaker, function (element) {
			...
			if (
				contains(targetNames, elementId) &&
				!contains(seen, elementId)
			) {
				seen.push(elementId);
				return true;
			}
	...
	function getMainContainer(param, context) {
		return isPanel(context)
			? context
			: new Window(param[0], param[1], param[2], param[3]);
	}
	function initBuildValues(resource, parserSelf) {
		var config = assign(
			clone(mainContainerDefault),
			baseGetConfig(resource)
		);
		...
		var layoutMode = assignLayoutMode(
			parserSelf.layoutMode,
			config.layoutMode
		);
	...
	function initMainContainer(resource, context) {
		var mainContainer = getMainContainer(
			assignWindowType(baseGetParam(resource)),
			context
		);
	...
	function isValidCombination(parentType, childType) {
		var flagCombination =
			elementTypeFlags[parentType] + elementTypeFlags[childType];
	...
	function parseElement(resource, container, collector) {
		forOwn(resource, function (value, key) {
			...
			if (
				isValidElement(flag) &&
				isValidCombination(container.type, flag)
			) {
				if (isContainer(flag)) {
					var newContainer = addContainer(
						container,
						value,
						flag,
						collector
	...
	function runInContext(resource) {
		...
		var container = buildWindow.apply(
			null,
			initBuildValues(resource_, tree)
		);
	...
	var textureWidth_dropDownList = elements.getElementById(
		'textureWidth_dropDownList'
	);
	var textureHeight_dropDownList = elements.getElementById(
		'textureHeight_dropDownList'
	);
	var textureName_dropDownList = elements.getElementById(
		'textureName_dropDownList'
	);
	...
	function refreshDigitsText() {
		digits_Statictext.text =
			'Digits: ' + toString(digits_Scrollbar.value.toFixed(0));
	}
	...
	function createComp() {
		...
		var compWidth =
			textureSizeArray[textureWidth_dropDownList.selection.index];
		var compHeight =
			textureSizeArray[textureHeight_dropDownList.selection.index];
		var parentFolderName =
			dataLeftCompleting(categoryFolderIndex, 2) +
			' ' +
			categoryFolderName;
		var parentFolder = getCategoryFolder(parentFolderName);
		var finalCompName = getFinalCompName(
			categoryFolderName,
			compWidth,
			compHeight,
			parentFolder
		);
		var targetComp = items.addComp(
			finalCompName,
			compWidth,
			compHeight,
			1,
	...
	function createTargetColorBg(targetColor, targetColorName) {
		...
		if (solidsFolder) {
			eachItems(solidsFolder, function (solidItem) {
				if (
					solidItem.name === bgName &&
					solidItem.comment === bgComment
				) {
					existBgSource = true;
	...
	function render() {
		...
		if (PNG_Checkbox.value) {
			...
			var targetOutputModule = targetRenderQueueItem.outputModule(
				numOutputModules++
			);
	...
	function getFinalCompName(compName, compWidth, compHeight, parentFolder) {
		...
		eachItems(parentFolder, function (compItem) {
			...
			if (
				compWidth == toNumber(compItemWidth) &&
				compHeight == toNumber(compItemHeight) &&
		...
		return getTargetCompName(
			...
			dataLeftCompleting(
				compIndex,
				toNumber(digits_Scrollbar.value.toFixed(0))
			)
		);
	}
	function applyTargetTemplate(targetOutputModule, targetTemplateName) {
		...
		var outputFolderPath =
			app.project.file.path + '//' + activeItem.parentFolder.name;
		var outputFile = new File(
			getFolder(outputFolderPath).fsName + '//' + activeItem.name
		);
